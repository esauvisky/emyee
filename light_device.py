...
class LightDevice:
    ...
    async def set_brightness(self, brightness: int, duration: float = 0.05):
        ...
        """
        if self.lock.locked():
            logger.debug(f"Brightness change ignored for {self.ip} because it's currently locked.")
            return

        await self.lock.acquire()
        try:
            logger.debug(f"Setting brightness to {brightness}% over {duration}s for {self.ip}")
            await asyncio.to_thread(
                self.bulb.set_brightness, brightness, duration=int(duration * 1000)
            )
            logger.debug(f"Brightness set to {brightness}% for {self.ip}")
        except asyncio.CancelledError:
            logger.debug(f"Brightness change task was cancelled for {self.ip}.")
            raise
    ...
    async def set_hsv(self, hue: int, saturation: int, brightness: int, duration: float = 0.05):
        ...
        """
        if self.lock.locked():
            logger.debug(f"HSV change attempting to override current state change for {self.ip}.")
            holder_task = self.lock.holder
            if holder_task and not holder_task.done():
                logger.debug(f"Cancelling current state change task for {self.ip}.")
                holder_task.cancel()
                try:
                    await holder_task
                except asyncio.CancelledError:
                    logger.debug(f"Cancelled task holding the lock for {self.ip}.")
                except Exception as e:
                    logger.error(f"Error while cancelling task for {self.ip}: {e}")

        await self.lock.acquire()
        try:
            logger.debug(f"Setting HSV to ({hue}, {saturation}, {brightness}) over {duration}s for {self.ip}")
            await asyncio.to_thread(
            ...
            logger.debug(f"HSV set to ({hue}, {saturation}, {brightness}) for {self.ip}")
        except asyncio.CancelledError:
            logger.debug(f"HSV change task was cancelled for {self.ip}.")
            raise
        except Exception as e:
    ...
    async def turn_on(self, duration: float = 0.05):
        ...
        """
        if self.lock.locked():
            logger.debug(f"Turn on ignored for {self.ip} because it's currently locked.")
            return

        await self.lock.acquire()
        try:
            logger.debug(f"Turning on bulb {self.ip} over {duration}s")
            await asyncio.to_thread(
                self.bulb.turn_on, duration=int(duration * 1000)
            )
            logger.debug(f"Bulb {self.ip} turned on")
        except asyncio.CancelledError:
            logger.debug(f"Turn on task was cancelled for {self.ip}.")
            raise
        except Exception as e:
    ...
    async def turn_off(self, duration: float = 0.05):
        ...
        """
        if self.lock.locked():
            logger.debug(f"Turn off ignored for {self.ip} because it's currently locked.")
            return

        await self.lock.acquire()
        try:
            logger.debug(f"Turning off bulb {self.ip} over {duration}s")
            await asyncio.to_thread(
                self.bulb.turn_off, duration=int(duration * 1000)
            )
            logger.debug(f"Bulb {self.ip} turned off")
        except asyncio.CancelledError:
            logger.debug(f"Turn off task was cancelled for {self.ip}.")
            raise
        except Exception as e:
